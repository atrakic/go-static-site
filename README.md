...
- `go.mod` is the source of truth for the Go toolchain version.
- CI uses `actions/setup-go` with `go-version-file: go.mod` to avoid version drift.
- Docker uses `ARG GO_VERSION` in `Dockerfile` (used by `FROM golang:${GO_VERSION}`) so Renovate can keep the builder image current.
